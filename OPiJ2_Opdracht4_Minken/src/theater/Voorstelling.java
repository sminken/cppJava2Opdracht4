...
public class Voorstelling {
   ...
   */
  public Voorstelling(String naam, GregorianCalendar datum) throws TheaterException {
    this.naam = naam;
    this.datum = datum;

//  Initialiseert de beginwaarden van de plaatsobjecten in de plaatsen Array.
   ...
   * @throws TheaterException 
   */
  public boolean plaatsKlant(int rijnummer, int stoelnummer, Klant klant) throws TheaterException {
    if (inZaal(rijnummer, stoelnummer)) {
  ...
  public void plaatsKlant(Klant klant) throws TheaterException {
    boolean gereserveerdePlaatsenAanwezig = false;
    for (int i = 1; i < plaatsen.length; i++) {
      for (int j = 1; j < plaatsen[i].length; j++) {
        if (plaatsen[i][j].getStatus().equals(Status.GERESERVEERD)) {
          gereserveerdePlaatsenAanwezig = true;
          plaatsen[i][j].plaatsKlant(klant);
          Voorstellingbeheer.schrijfPlaats(datum, i, j, klant.getKlantnummer());
        }
      }
    }
    if (!gereserveerdePlaatsenAanwezig) {
      throw new TheaterException("Er zijn geen te reserveren plaatsen geselecteerd");
    }
  }
